...
import { updateHealthMessage } from './health.js';
  ...
  if (!run.stock) {
    if (run.lastError) {
      return {
        ...
        icon: '🔴',
        label: 'Blocked',
        reason: run.waitingReason || 'The last automatic check failed',
      };
    }
    return { type, icon: '⚪', label: 'Waiting', reason: 'Checking stock' };
  }
...
async function sendHealthAlert(client, run, reason) {
  ...
  console.warn(`Auto-generation health alert for guild ${run.guildId}: ${reason}`);
  await updateHealthMessage(client, run.guildId);
}
...
async function generateNext(client, run) {
  ...
  try {
    ...
    const errorMessage = err.message || 'The BloxGen check failed.';
    const missingSetup = /personal BloxGen API key|SESSION_SECRET|add your personal/i.test(errorMessage);
    run.waitingReason = missingSetup
      ? 'Add a personal BloxGen API key with `/key`; auto-generation will retry automatically.'
      : `BloxGen check failed: ${errorMessage}`;
