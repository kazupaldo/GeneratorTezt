...
export async function ensureDeliveryReady({
  client,
  guildId,
  fallbackChannel,
  user,
  ...
  if (forcePrivate || mode === 'dm' || mode === 'both') {
    await user.createDM();
  }
...
export async function deliverAccount({
  ...
  const mode = getDelivery(guildId);
  const forcePrivate = Boolean(payload?.forcePrivate);
  const effectiveMode = forcePrivate && mode === 'server' ? 'both' : mode;
  const result = {
    mode: effectiveMode,
    channelSent: false,
    dmSent: false,
  ...
  if (effectiveMode === 'server' || effectiveMode === 'both') {
    try {
      const channel = await resolveChannel(client, guildId, fallbackChannel, type);
  ...
  if (effectiveMode === 'dm' || effectiveMode === 'both') {
    try {
      const item = {
        guildId,
        type,
