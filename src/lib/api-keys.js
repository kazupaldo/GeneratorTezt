...
export function hasPersonalApiKey(userId) {
  return Boolean(userId && Object.keys(recordsFor(userId).keys).length);
}

export function getApiKeyStorageStatus() {
  return {
    ready: Boolean(ENCRYPTION_KEY),
    message: ENCRYPTION_KEY
      ? 'Personal keys can be encrypted and saved.'
      : 'SESSION_SECRET is missing. An administrator must add it before personal keys can be saved.',
  };
}
...
export function setUserApiKey(userId, apiKey, name = 'default') {
  ...
  if (normalizedApiKey.length < 8) {
    throw new Error('The BloxGen API key is empty or too short. Paste the complete key and try again.');
  }
  const record = recordsFor(userId);
  const safeName = String(name || 'default').trim().toLowerCase().replace(/[^a-z0-9_-]/g, '-').slice(0, 32) || 'default';
  cache[userId] = {
    active: record.active || safeName,
    keys: { ...record.keys, [safeName]: encrypt(normalizedApiKey) },
  };
  save();
  return safeName;
}
