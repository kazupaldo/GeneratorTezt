...
import { DISCORD_TOKEN, PREFIX, SESSION_SECRET } from './config.js';
...
import { getAutoPasswordConfig } from './lib/settings.js';
...
client.once('clientReady', async (c) => {
  ...
  if (!SESSION_SECRET) {
    console.error('SESSION_SECRET is missing. Personal API keys cannot be saved and auto-generation will remain blocked until it is configured.');
  }
  ...
  const refreshHealth = async () => {
    for (const guild of c.guilds.cache.values()) {
      await updateHealthMessage(c, guild.id, {
        autoPassword: getAutoPasswordConfig(guild.id).enabled,
      });
    }
  };
  await refreshHealth();
