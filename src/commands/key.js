...
export function buildApiKeyPanel() {
  const storage = getApiKeyStorageStatus();
  ...
  return {
    content:
      ...
      `${storage.ready ? '✅ Secure key storage is ready.' : `⚠️ **Cannot save keys yet:** ${storage.message}`}`,
    components: [buttons],
  };
}
...
export default {
  ...
  async execute({ interaction, message, args = [] }) {
    // Slash-command replies are ephemeral, so keep the complete setup panel
    // in the current interaction instead of making the user hunt through DMs.
    // Prefix commands still use a DM because normal message replies cannot be
    // private to one member.
    if (interaction) return buildApiKeyPanel();
    ...
    if (action === 'status') {
      return keys.names.length
        ...
        : '📭 No personal BloxGen API key is saved yet.';
    }
