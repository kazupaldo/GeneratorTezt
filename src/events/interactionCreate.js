...
async function handlePasswordChange(interaction, parsed) {
  ...
  try {
     const account = getUpdatedAccount(parsed.username) ?? await findAccountByUsername(parsed.username);
    if (!account) {
...
async function handleChatInputCommand(interaction, client) {
  ...
  switch (interaction.commandName) {
    ...
    case 'autopassword': {
      args = [options.getString('action') || 'status'];
      const type = options.getString('type');
      const channel = options.getChannel('channel');
...
export async function execute(interaction) {
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (value.length < 8) {
        await interaction.reply({
          content: '❌ The BloxGen API key is empty or too short. Paste the complete key and try again.',
          flags: MessageFlags.Ephemeral,
        });
        return;
      }
